...
- `server.js` - Node static server plus OAuth-gated `/api/signatures` and
  `/api/profile` APIs. Signed-in users (Google sign-in via xhost) can store up
  to 20 named signatures and a reusable auto-fill profile (name, email, address,
  phone, initials, date preference) in Postgres. Every data path requires a
